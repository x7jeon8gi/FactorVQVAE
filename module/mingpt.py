...
class MarketAttention(nn.Module):
    """
    Market Attention layer for incorporating market information into the model.
    ...
    def __init__(self, config):
        ...
        # Causal Mask: block_size x block_size
...
class GPT(nn.Module):
    ...
    def forward(self, idx, market=None, embeddings=None):
        """
        idx: (B, T) integer indices (tokens)
        market: (B, T, market_dim) market information
        embeddings: additional embeddings to concatenate (e.g., past context)
        """
        # 1) Token embeddings
        token_embeddings = self.tok_emb(idx)  # (B, T, C)
        
        # 2) Optionally concatenate additional embeddings
        if embeddings is not None:
            token_embeddings = torch.cat((embeddings, token_embeddings), dim=1)

        T = token_embeddings.size(1)
        assert T <= self.block_size, "Input length exceeds block_size."

        # 3) Add positional embeddings
        position_embeddings = self.pos_emb[:, :T, :]
        x = self.drop(token_embeddings + position_embeddings)

        # 4) Iterate over Transformer Blocks
        for block in self.blocks:
            x = block(x, market=market)

        # 5) Final LayerNorm + logits
        x = self.ln_f(x)
        logits = self.head(x)
